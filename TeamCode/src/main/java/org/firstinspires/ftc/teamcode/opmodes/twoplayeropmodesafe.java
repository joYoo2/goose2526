...
@TeleOp(name = "Two Player OpMode (Safe)")
public class twoplayeropmodesafe extends CommandOpMode {
    ...
    private boolean intakeRunning = false;    // Tracks if intake is currently running
    ...
    // Shooter speed reduction to prevent power issues
    private static final double SHOOTER_INTAKE_SPEED_MULTIPLIER = 0.7; // Run at 70% when intake active (shooting)
    private static final double SHOOTER_IDLE_SPEED_MULTIPLIER = 0.85;  // Run at 85% when pre-spinning (not shooting)
    ...
    static {
        ...
        lookUpAutoShoot.add(11.5, 1020);
        lookUpAutoShoot.add(12.0, 1040);

        lookUpAutoShoot.add(13.0, 1050);
        lookUpAutoShoot.add(14.0, 1070);
        lookUpAutoShoot.add(15.0, 1100);
        lookUpAutoShoot.add(10000.0, 1100);
    ...
    public void run() {
        ...
        // Reduce speed to prevent power issues:
        // - Full speed only when actively shooting (left trigger + shootingEnabled + not intakeRunning yet)
        // - 70% when intake is running (actively feeding balls)
        // - 85% when just pre-spinning (idle, not shooting)
        boolean activelyShooting = gamepad1.left_trigger > 0.5 && shootingEnabled;
        if(shooterSpinning){
            int effectiveSpeed = shooterSpeed + adjustSpeed;
            if (intakeRunning) {
                // Lowest power: intake motors are running
                effectiveSpeed = (int)(effectiveSpeed * SHOOTER_INTAKE_SPEED_MULTIPLIER);
            } else if (!activelyShooting) {
                // Medium power: pre-spinning but not shooting
                effectiveSpeed = (int)(effectiveSpeed * SHOOTER_IDLE_SPEED_MULTIPLIER);
            }
            // Full power: shooting mode active but intake not yet running (ramping up)
            robot.shooter.shoot(effectiveSpeed);
        }else{
            robot.shooter.stop();
        }
        ...
        String shooterMode = intakeRunning ? "INTAKE (70%)" : (!activelyShooting ? "IDLE (85%)" : "SHOOTING (100%)");
