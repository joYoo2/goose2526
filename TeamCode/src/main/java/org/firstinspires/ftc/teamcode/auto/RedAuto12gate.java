 ...
 * 
 * Features:
 * - Turret auto-aiming at future pose before each shooting sequence
 * - Hood angle auto-calculated from LUT
 * - Manual intake lift control for gate operations
 * - Stopper controlled via Globe.shootingActive flag
 * - Mirrored from BlueAuto for Red alliance
 */
@Autonomous(name = "Red Close 15 Gate", group = "Competition")
public class RedAuto12gate extends CommandOpMode {
    ...
    // Path poses - mirrored from BlueAuto
    private final Pose startPose = new Pose(19, 116, Math.toRadians(180)).mirror();
    private final Pose shootPose = new Pose(50, 90, Math.toRadians(180)).mirror();
    private final Pose middlePose = new Pose(8.5, 60, Math.toRadians(180)).mirror();
    private final Pose tapGate = new Pose(11, 63.5, Math.toRadians(145)).mirror();
    private final Pose topPose = new Pose(14, 84, Math.toRadians(180)).mirror();
    private final Pose bottomPose = new Pose(12, 36, Math.toRadians(180)).mirror();
    private final Pose shootPose2 = new Pose(56, 120, Math.toRadians(180)).mirror();
    private final Pose park = new Pose(56, 120, Math.toRadians(180)).mirror();
    ...
    public void generatePaths() {
        ...
        tapTheGate = robot.follower.pathBuilder()
                .addPath(new BezierCurve(shootPose, new Pose(72, 24).mirror(), tapGate))
                .setLinearHeadingInterpolation(shootPose.getHeading(), tapGate.getHeading())
                .build();

        shootGate = robot.follower.pathBuilder()
                .addPath(new BezierCurve(tapGate, new Pose(70, 38).mirror(), shootPose))
        ...
        getBottom = robot.follower.pathBuilder()
                .addPath(new BezierCurve(shootPose, new Pose(76.25, 18).mirror(), bottomPose))
                .setConstantHeadingInterpolation(shootPose.getHeading())
                .build();
